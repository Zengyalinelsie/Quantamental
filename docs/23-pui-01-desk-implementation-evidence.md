...
| `docs/18` 响应式表（原值） | 224 px |
| Figma node `3:398` 实测 | 248 px |
| **裁决结果（2026-08-15）** | **280 px 展开 / 72 px 收起** |

本工作包**新增了第三个数据点**（Figma 实测 248 px），使原有两值冲突升级为三值。
用户 2026-08-15 明确裁决采用 **280 px**，冲突已收口：

- `docs/18` 响应式表已从 224 px 更新为 280 px，224 px 不再是有效记录；
- `CLAUDE.md` 已从"尚未裁决的设计冲突"改为已裁决记录；
- SPEC-045 无需修改，运行时无需修改。

保留的已批准差异：1440 下主内容区为 **1160 px** 而非 Figma 的 1192 px，差 32 px。两栏以比例
`minmax(0, 740fr) minmax(0, 380fr)` 声明，使差异被两栏吸收而不产生水平溢出。

**因此 Desk 的 Design Parity 记为 `parity_verified_with_known_deviation`**：结构、分栏、
字号层级与 token 均对照 node `3:398`，但侧栏宽度按已批准决定偏离设计稿。后续页面按同一比例
换算，不得为对齐 1192 px 而改回 248 px。
...
- 侧栏宽度三值冲突 → 2026-08-15 裁决为 280 px，`docs/18` 与 `CLAUDE.md` 已同步（见 §6.1）；
- Figma 设计输入阻断 → 17 个关键 Frame 精确资产已入库（见 §3）。
