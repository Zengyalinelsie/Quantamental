...
|---|---|---|---|---|
| 1440 | 280 px 左导航；完整 Data Mode/Stage/Time/Universe | 12 列，高密度双栏或四栏 | 完整列、证据侧栏 | 页内右上与底部 Gate 均可见 |
...
> 侧栏宽度冲突已裁决（2026-08-15）：桌面展开侧栏统一为 **280 px**，收起 72 px，与 SPEC-045 一致。
>
> 该冲突一度有三个值：SPEC-045 规定 280 px，本表原写 224 px，而 Figma `desk-daily-workstation`
> （node `3:398`）实测为 248 px。用户 2026-08-15 明确裁决采用 280 px，本表已同步更新，
> 224 px 不再是有效记录。
>
> 因此 1440 下主内容区为 1160 px，比 Figma Frame 的 1192 px 窄 32 px。这是**已批准的设计差异**：
> 页面栅格以比例声明，使该差异被内容列吸收而不产生水平溢出。按精确 Figma 节点实现页面时应按此
> 比例换算，不得为对齐 1192 px 而改回 248 px。
